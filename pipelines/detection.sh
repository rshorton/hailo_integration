...
if [[ $input_source =~ "/dev/video" ]]; then
    ...
    source_element="rosimagesrc ros-topic=\"/color/image\""
else
...
PIPELINE="${debug_stats_export} gst-launch-1.0 ${stats_element} \
    ...
    queue leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! \
    hailonet hef-path=$hef_path $device_id_prop batch-size=$batch_size $thresholds_str multi-process-service=false ! \
    queue leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! \
