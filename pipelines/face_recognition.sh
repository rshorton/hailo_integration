...
function main() {
    ...
    RECOGNITION_PIPELINE="hailocropper so-path=$CROPPER_SO function-name=face_recognition internal-offset=true name=cropper2 \
        ...
        cropper2. ! \
            ...
            hailonet hef-path=$recognition_hef scheduling-algorithm=1 vdevice-key=$vdevice_key multi-process-service=true ! \
    ...
    FACE_DETECTION_PIPELINE="hailonet hef-path=$hef_path scheduling-algorithm=1 vdevice-key=$vdevice_key multi-process-service=true ! \
    ...
    pipeline="gst-launch-1.0 \
        ...
        hailoexportzmq qos=false address="tcp://127.0.0.1:5558" ! \
